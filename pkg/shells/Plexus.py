...
class Plexus(pyre.plexus, family="smith.shells.plexus"):
    """
    ...
    def pyre_banner(self):
        ...
        """
        # the project header
        yield from textwrap.dedent(smith.meta.banner).splitlines()
        # the doc string
        yield from self.pyre_showSummary(indent="")
        # the authors
        yield from textwrap.dedent(smith.meta.authors).splitlines()
        # all done
        return

    # interactive session management
    def pyre_interactiveSessionContext(self, context: dict | None = None):
        """
        Go interactive
        """
        ...
        context["smith"] = smith  # my package
        # and chain up
    ...
    def pyre_mountApplicationFolders(self, pfs, prefix, **kwds):
        ...
        # if all goes well and we reach the intended folder without errors
            ...
            self._ux = smith.ux.dispatcher(plexus=self, docroot=docroot, pfs=pfs)
